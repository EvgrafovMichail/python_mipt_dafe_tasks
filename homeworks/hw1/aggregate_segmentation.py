...
def is_valid_segment(segment: dict[str, float | str]) -> bool:
    ...
    if not segment["type"] and not segment["segment_start"] and not segment["segment_end"]:
        return True
    if not isinstance(type, str) or not isinstance(start, float) or not isinstance(end, float):
        return False
...
def aggregate_segmentation(
    ...
    for segment in segmentation_data:
        ...
        if segment_id in res_dct[audio_id]:
            ...
            if (
                now_segment["end"] != past_segment["end"]
                or now_segment["start"] != past_segment["start"]
                or now_segment["type"] != past_segment["type"]
            ):
                flag_dct[audio_id] = False
                continue

        else:
            res_dct[audio_id].update(
                {
                    segment_id: {
